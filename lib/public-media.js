...
async function resolveReleaseAssetDownloadUrl(assetUrl) {
  ...
  if (!getResponse.ok || !getResponse.url) {
    throw new Error(`Could not resolve a public GitHub asset URL from ${assetUrl}.`);
  }
...
async function prepareGitHubReleaseMedia(env, auth, file) {
  ...
  try {
    ...
    const resolvedUrl = await resolveReleaseAssetDownloadUrl(asset.browser_download_url);
