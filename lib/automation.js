...
const { preparePublicMedia } = require("./public-media");
const {
...
async function getStatus(options = {}) {
  ...
  return {
    ...
    stateBackend: state.postingState.backend || state.env.postingStateBackend,
...
async function runOnce(options = {}) {
  ...
  if (effectiveDryRun || options.previewOnly) {
    return {
      ...
      uploadMode: isInstagramLogin ? state.env.publicMediaProvider : "resumable-upload",
      postedHandling: state.config.queue.postedHandling,
      stateBackend: state.postingState.backend || state.env.postingStateBackend
    };
  }
  ...
  try {
    if (isInstagramLogin) {
      publicMedia = await preparePublicMedia(state.env, state.google.auth, file);
    }

    const container = await createMediaContainer(state.env, state.config, captionPackage.caption, {
      videoUrl: publicMedia ? publicMedia.url : undefined
    });
    ...
    if (publicMedia && typeof publicMedia.cleanup === "function") {
      try {
        await publicMedia.cleanup();
      } catch (cleanupError) {
        console.error("Failed to clean up public media asset.", cleanupError);
      }
    }
  }
}
