...
const EnvSchema = z.object({
  ...
  POSTING_STATE_BACKEND: z.string().optional(),
  POSTING_STATE_FILE_PATH: z.string().optional(),
  PUBLIC_MEDIA_PROVIDER: z.string().optional(),
  ...
  GITHUB_MEDIA_REPOSITORY: z.string().optional(),
  GITHUB_MEDIA_RELEASE_TAG: z.string().optional(),
  GITHUB_MEDIA_RELEASE_NAME: z.string().optional(),
  GITHUB_MEDIA_ASSET_PREFIX: z.string().optional(),
...
function loadEnv() {
  ...
  cachedEnv = {
    ...
    publicMediaProvider: String(parsed.PUBLIC_MEDIA_PROVIDER || "vercel-proxy").trim().toLowerCase(),
    githubToken: String(parsed.GITHUB_TOKEN || "").trim(),
    githubMediaRepository: String(
      parsed.GITHUB_MEDIA_REPOSITORY || parsed.GITHUB_REPOSITORY || ""
    ).trim(),
    githubMediaReleaseTag: String(parsed.GITHUB_MEDIA_RELEASE_TAG || "automation-media").trim(),
    githubMediaReleaseName: String(
      parsed.GITHUB_MEDIA_RELEASE_NAME || "Automation Media"
    ).trim(),
    githubMediaAssetPrefix: String(
      parsed.GITHUB_MEDIA_ASSET_PREFIX || "automation-media"
    ).trim(),
    postingStateBackend: String(parsed.POSTING_STATE_BACKEND || "drive-file").trim().toLowerCase(),
    postingStateFilePath: String(parsed.POSTING_STATE_FILE_PATH || ".automation/posting-state.json").trim(),
