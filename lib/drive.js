...
function resolveLocalStateFilePath(env) {
  return path.resolve(process.cwd(), env.postingStateFilePath || ".automation/posting-state.json");
}

async function loadLocalPostingState(env) {
  const filePath = resolveLocalStateFilePath(env);
  ...
  try {
    ...
    return {
      ...
      filePath,
      state: {
        postedFileIds: Array.isArray(parsed.postedFileIds)
          ? [...new Set(parsed.postedFileIds.map((value) => String(value)))]
          : [],
...
async function saveLocalPostingState(env, state) {
  const filePath = resolveLocalStateFilePath(env);
...
async function loadPostingState(drive, env, config) {
  if (env.postingStateBackend === "local-file") {
    return loadLocalPostingState(env);
  }
...
async function markFileAsPosted(drive, env, config, postingState, file, publishResult) {
  ...
  if (env.postingStateBackend === "local-file") {
    const filePath = await saveLocalPostingState(env, nextState);
    return {
      backend: "local-file",
      fileId: null,
      folderId: null,
